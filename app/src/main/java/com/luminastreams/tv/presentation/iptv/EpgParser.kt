...
private fun skipXmlPreamble(input: InputStream): InputStream {
    ...
    if (read <= 0) return pb
    var skip = 0
    if (read >= 3 && header[0] == 0xEF.toByte() && header[1] == 0xBB.toByte() && header[2] == 0xBF.toByte()) skip = 3
    else if (read >= 2 && ((header[0] == 0xFF.toByte() && header[1] == 0xFE.toByte()) || (header[0] == 0xFE.toByte() && header[1] == 0xFF.toByte()))) skip = 2
...
private class XmlTvHandler(
    ...
    // יצירה חד פעמית של הלוח כדי למנוע קריסת זיכרון
    private val sharedCalendar = Calendar.getInstance(TimeZone.getTimeZone("UTC"))
    ...
    private fun parseTimeFast(s: String): Long {
        ...
        return try {
            ...
            sharedCalendar.clear()
            sharedCalendar.set(y, mo - 1, d, h, m, sec)
            ...
            sharedCalendar.timeInMillis - offset
